...
async function detail(id) {
    ...
    let allSources = _.map(info.vod_play_list, item => {
        const playerInfo = item.player_info || {};
        const parse = playerInfo.parse || '';
        const ua = playerInfo.user_agent || customPlayUa || '';
        const show = (playerInfo.show || '').toLowerCase();
        const urls = _.map(item.urls || [], it => {
            const { name = '', url = '', token = '', parse_api_url = '', nid = 1 } = it;
    ...
    allSources.forEach(s => {
        if (s.show.includes('4k') || s.show.includes('k')) {
            fourKList.push(s.urls);
        } else if (s.show.includes('蓝光') || s.show.includes('秒播') || s.show.includes('专线') || s.show.includes('自建')) {
            blueList.push(s.urls);
        }
    });


    let finalBlue = '';
    for (let u of blueList) {
        if (u && u.trim()) {
            finalBlue = u;
            break;
        }
    ...
    if (finalBlue) {
        playFrom.push('恒轩');
        playUrl.push(finalBlue);
    }
    if (final4K) {
        playFrom.push('4K');
        playUrl.push(final4K);
    }

    videos.vod_play_from = playFrom.join('$$$');
    videos.vod_play_url = playUrl.join('$$$');
