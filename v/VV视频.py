...
class Spider(Spider):
    WEB_HOST = 'shturl.cc/660m9QEUEs3cZXcLgQQ'
    ENTRY_PATH = '/video'
    ...
    def _play_sources(self, item):
        ...
        valid_sources = [s for s in sources if s.get('url', '').startswith(('http://', 'https://'))]
        if len(valid_sources) >= 2:
            return valid_sources[1]
