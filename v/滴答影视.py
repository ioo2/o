    ...
    def detailContent(self, ids):
        ...
        for i,g in enumerate(groups):
            ...
            for h,n in re.findall(r'<a\b[^>]+href=["\']([^"\']*/play/\d+-\d+-\d+\.html)["\'][^>]*>([\s\S]*?)</a>', g, re.I):
                ...
                eps.append(name + '$' + fu)
            if eps:
                line_name = names[i] if i < len(names) and names[i] else '线路%d' % (i+1)
          
                if re.search(r'网盘|云盘|夸克|百度|UC|PikPak|阿里', line_name, re.I):
                    continue
                play_from_all.append(line_name)
                play_url_all.append('#'.join(eps))
  
        if not play_from_all:
            eps=[]
            ...
            if eps:
                play_from_all = ['默认']
                play_url_all = ['#'.join(eps)]

        
        target_eps = ""
       
        find_4k = False
        for idx, fname in enumerate(play_from_all):
            if re.search(r'4K|蓝光', fname, re.I):
                target_eps = play_url_all[idx]
                find_4k = True
                break
      
        if not find_4k and len(play_url_all) > 0:
            target_eps = play_url_all[0]
        
        final_play_from = ["恒轩"]
        final_play_url = [target_eps] if target_eps else []
     

        vod = {
            'vod_id':url,
            'vod_name':title,
            'vod_pic':self.fix(picm.group(1)) if picm else '',
            'type_name':info('分类'),
            'vod_year':info('年份')[:4],
            'vod_area':info('地区'),
            'vod_remarks':info('更新时间'),
            'vod_actor':info('主演'),
