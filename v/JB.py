...
class Spider(Spider):
    ...
    headers = {
        'User-Agent': 'Mozilla/5.0 (iPad; CPU OS 16_6 like Mac OS X) AppleWebKit/605.1.5 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1',
    ...
    def detailContent(self, ids):
        ...
        for line in line_priority1:
            for idx, line_name_ele in enumerate(nd):
                ...
                if current_line == line and pd[idx]('li').items():
                    play_url1 = '#'.join([f"{j.text()}${j('a').attr('href')}" for j in pd[idx]('li').items()])
                    break
            if play_url1:
                break
        if play_url1:
        ...
        for line in line_priority2:
            for idx, line_name_ele in enumerate(nd):
                current_line = line_name_ele.text().strip()
                if current_line == line and pd[idx]('li').items():
                    play_url2 = '#'.join([f"{j.text()}${j('a').attr('href')}" for j in pd[idx]('li').items()])
                    break
            if play_url2:
                break
        if play_url2:
        ...
        # 组合播放线路
        if play_list:
            vod['vod_play_from'] = '$$$'.join([p[0] for p in play_list])
            vod['vod_play_url'] = '$$$'.join([p[1] for p in play_list])
        else:
            vod['vod_play_from'] = ''
            vod['vod_play_url'] = ''
