...
class Spider(BaseSpider):
    CONFIG = {
        ...
        "host": "",
        "site": "https://raw.giteeusercontent.com/dream-rain-star-destiny_0/raw.giteeusercon.com/raw/master/aa/app_xpg_android.txt",
        "version": "1.0.0.3",
        "decrypt": "0"
    }
    def getName(self):
    ...
    def _device(self):
        ...
        return {
            ...
            "lang": "zh", "brand": "Xiaomi", "density": "2.75", "appName": self.cfg.get("appName", "苹果影视"),
    ...
    def detailContent(self, ids):
        try:
            ...
            select_lines = None
            priority_keys = ["4k专线2"]
            for pri_name in priority_keys:
                for k, v in sources.items():
                    if k.startswith(pri_name) and v and len(v) > 0:
                        select_lines = v
                        break
                if select_lines is not None:
                    break
            ...
            vod = {
                ...
                "vod_play_from": "$$$".join(final_sources.keys()),
                "vod_play_url": "$$$".join("#".join(v) for v in final_sources.values())
            }
            return {"list": [vod]}
        except Exception as e:
