...
async function detail(id) {
    ...
    _.map(info.vod_play_list, item => {
        ...
        const nameUrls = _.map(urls, item2 => {
        ...
        allSources.push({
            ...
            urls: nameUrls,
            hasData: urls.length > 0
        });
    });

    let target = null;
    for (let s of allSources) {
        if (s.show.includes('4K线路③') && s.hasData) {
            target = s;
            break;
        }
    }
    ...
    if (!target && allSources.length > 0) {
        target = allSources[0];
    }

    videos.vod_play_from = '恒轩';
    videos.vod_play_url = target ? target.urls : '';
