...
class Spider(Spider):
    ...
    def detailContent(self, ids):
        ...
        if links:
            ...
            if ph:
                source_items = re.findall(r'<a[^>]*class="source-item[^"]*"[^>]*>(.*?)</a>', ph, re.S)
                ep_divs = re.findall(r'<div class="episode-list"[^>]*>(.*?)</div>', ph, re.S)
                max_loop = min(len(source_items), len(ep_divs))
                for idx in range(max_loop):
                    ...
                    nm = '%s(%s)' % (lb, sb) if lb and sb else lb
                    nm_norm = unicodedata.normalize('NFKC', nm)
                    if self._is_wm(nm_norm) or any(k in nm_norm.lower() for k in self.skip):
                        continue
                    if '蓝光7' in nm_norm or '高清7' in nm_norm:
                        continue
                    eps = []
                    ...
                    if not eps:
                        continue
                    ep_str = '#'.join(eps)
                    candidate_list.append((nm_norm, ep_str))
        final_ep = ''
        for name_norm, ep_str in candidate_list:
            if '超清2' in name_norm and ('秒播' in name_norm or '4K' in name_norm):
                final_ep = ep_str
                break
        if not final_ep:
            for name_norm, ep_str in candidate_list:
                if '蓝光' in name_norm or '高清' in name_norm:
                    final_ep = ep_str
                    break
        if not final_ep and candidate_list:
            final_ep = candidate_list[0][1]
        if not final_ep and links:
            final_ep = '#'.join('第%d集$%s' % (i+1, u) for i,u in enumerate(links))
        vod['vod_play_from'] = '恒轩'
        vod['vod_play_url'] = final_ep
        return {'list': [vod]}
