...
Modified: Only one play line named 恒轩, priority high-def source with episodes, vertical poster ratio 0.67
"""
import base64
import html
import json
...
class Spider(Spider):
    ...
    DEFAULT_CLASSES = (
        ...
        ('动漫', '/page/guochandongman.html'),
        ('日漫', '/page/ribendongman.html'),
    ...
    def _playlists(self, data, page_url):
        ...
        for index, block in enumerate(blocks, start=1):
            ...
            if episodes:
                temp_from.append(self._safe_part(line, '线路%d' % index))
                temp_urls.append('#'.join(episodes))
        if not temp_urls:
            groups = {}
            ...
            for sid, episodes in groups.items():
                line_name = f"线路{sid}"
                if "在线观看" in line_name:
                    continue
                temp_from.append(line_name)
                temp_urls.append('#'.join(dict.fromkeys(episodes)))
        valid_pairs = []
        for f, u in zip(temp_from, temp_urls):
            if "在线观看" in f:
                continue
            if u and len(u.strip()) > 0:
                valid_pairs.append((f, u))
        if not valid_pairs:
            return [], []
        first_valid_url = valid_pairs[0][1]
        final_from = ["恒轩"]
        final_urls = [first_valid_url]
        return final_from, final_urls
