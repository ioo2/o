...
class Spider(BaseSpider):
    ...
    def getName(self):return "橘汁"
    def init(self, extend=""):
        self.cfg = dict(self.CONFIG)
        if isinstance(extend, dict):self.cfg.update(extend)
        elif isinstance(extend, str) and extend.strip():
            try:self.cfg.update(json.loads(extend))
        ...
        if site and self.session and not self.host:
            try:
                ...
                if domain:self.host = domain
            except Exception:pass
        if self.host:
            try:self._load_dynamic_key()
            except Exception:self.dynamic_key = ""
    ...
    def _varint(value):
        ...
        return bytes(out)
    @classmethod
    def _pb_int(cls,field,value):return cls._varint(field<<3)+cls._varint(value)
    @classmethod
    ...
    def _read_varint(data,pos):
        ...
        while pos<len(data):
            ...
            shift+=7
            if shift>70:raise ValueError("bad protobuf varint")
    ...
    def _pb_parse(cls,data):
        ...
        while pos<len(data):
            ...
            if wire==0:value,pos=cls._read_varint(data,pos)
            elif wire==1:value=data[pos:pos+8];pos+=8
            elif wire==2:
                size,pos=cls._read_varint(data,pos)
                value=data[pos:pos+size];pos+=size
            elif wire==5:value=data[pos:pos+4];pos+=4
            else:raise ValueError("unsupported protobuf wire type: %s"%wire)
    ...
    @staticmethod
    def _check_crypto():
        if AES is None or RSA is None:raise RuntimeError("缺少 pycryptodome")
    @classmethod
    ...
    def _post(self,path,body):
        if not self.session or not self.host:raise RuntimeError("橘汁接口域名获取失败")
    ...
    def _get_json(self,path):
        ...
        r.raise_for_status()
        return r.json()
    def _api_data(self,raw):return self._first(self._pb_parse(raw),3,b"")
    ...
    def homeContent(self,filter):
        try:
            ...
            classes,filters=[],{}
            allow_name={"剧集","动漫","电影","综艺"}
            for item in obj.get("data") or []:
                if str(item.get("name",""))=="公告":continue
                tid,name=str(item.get("id","")),str(item.get("name",""))
                if not tid or name not in allow_name:continue
                ...
                try:
                    ...
                    if isinstance(ext_data,dict):
                        mapping=[("movieClass","class","类型"),("area","area","地区"),("lang","lang","语言"),("year","year","年份"),("extendSort","extend_sort","排序")]
                ...
                if not fl:
                    ...
                    try:ext=json.loads(raw_filter) if isinstance(raw_filter,str) else raw_filter
                ...
                if fl:filters[tid]=fl
            return {"class":classes,"filters":filters}
        except Exception as e:return {"class":[],"filters":{},"error":str(e)}
    def homeVideoContent(self):
        videos=[];err_msg=""
        try:
            obj=self._get_json("/api/ex/v3/security/tag/list")
            raw_data=obj.get("data","")
            if isinstance(raw_data,(list,dict)):arr=[raw_data] if isinstance(raw_data,dict) else raw_data
            else:
                data_str=str(raw_data) if raw_data else "";arr=None
                if data_str:
                    try:arr=json.loads(data_str)
                    except Exception:pass
                if arr is None and str(self.cfg.get("decrypt","1"))!="0" and data_str:
                    try:arr=json.loads(self._aes_decrypt_ecb(data_str,self.data_key))
                    except Exception:pass
                if arr is None and str(self.cfg.get("decrypt","1"))!="0" and data_str:
                    try:
                        d1=self._aes_decrypt_ecb(data_str,self.data_key)
                        d2=self._aes_decrypt_ecb(d1,self.data_iv)
                        arr=json.loads(d2)
                    except Exception as e2:err_msg="双层解密失败: "+str(e2)
            if arr is None:arr=[]
            for block in arr or []:
                if not isinstance(block,dict):continue
                if "vodList" in block:
                    for vod in block.get("vodList") or []:self._append_vod(videos,vod)
                    continue
                for section in block.get("sections") or []:
                    if not isinstance(section,dict):continue
                    for vod in section.get("vodList") or []:self._append_vod(videos,vod)
            if videos:return {"list":videos}
        except Exception as e:err_msg="推荐接口异常: "+str(e)
        try:
            ...
            if classes:
                ...
                if first_tid:
                    ...
                    if fallback:return {"list":fallback,"note":"首页推荐降级至分类内容"}
        except Exception as e2:err_msg+=" | 降级也失败: "+str(e2)
        return {"list":videos,"error":err_msg}
    def _append_vod(self,videos,vod):
        if not isinstance(vod,dict):return
        pic=""
        cover=vod.get("coverImage") or vod.get("cover") or {}
        if isinstance(cover,dict):pic=cover.get("path") or cover.get("thumb") or cover.get("url") or ""
        videos.append({
    ...
    def detailContent(self,ids):
        try:
            ...
            for item in f.get(29,[]):
                ...
                source_dict.setdefault(source,[]).append((video["title"] or "播放")+"$"+token)
            if "自建4K" in source_dict:
                select_lines = source_dict["自建4K"]
            else:
                select_lines=next(iter(source_dict.values()),[])
            vod={
                ...
                "vod_play_from":"恒轩",
                "vod_play_url":"#".join(select_lines)
            }
            return {"list":[vod]}
        except Exception as e:return {"list":[],"error":str(e)}
    ...
    def playerContent(self,flag,id,vipFlags):
        try:
            if self.isVideoFormat(id):return {"parse":0,"url":id,"header":{}}
        ...
        except Exception as e:return {"parse":1,"url":id,"header":{},"error":str(e)}
    def localProxy(self,param):return [404,"text/plain","",None]