    ...
    def detailContent(self, ids):
        ...
        try:
            ...
            for ln_name in lines:
                ...
                lines[ln_name] = norm
            target_line = None
            if '蓝光4K' in lines and len(lines['蓝光4K'])>0:
                target_line = lines['蓝光4K']
            else:
                for lname in lines:
                    if len(lines[lname])>0:
                        target_line = lines[lname]
                        break
            if target_line:
                out['vod_play_from'] = '恒轩'
                out['vod_play_url'] = '#'.join(target_line)
            else:
                out['vod_play_from'] = '恒轩'
