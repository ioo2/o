        ...
        try:
            ...
            if html:
                ...
                if data.get("code") == 200 and data.get("data"):
                    ...
                    target_idx = -1
                    backup_idx = -1
                    for idx,f in enumerate(from_list):
                        if f == "CO4K":
                            eps_tmp = url_list[idx].split("#")
                            if eps_tmp and eps_tmp[0]:
                                target_idx = idx
                        if backup_idx == -1 and url_list[idx]:
                            backup_idx = idx
                    use_idx = target_idx if target_idx != -1 else backup_idx
                    play_from = []
                    play_url = []
                    vod_id_str = str(vod.get("vod_id", ""))
                    if use_idx != -1 and use_idx < len(url_list):
                        eps = url_list[use_idx].split("#")
                        urls = []
                        for idx, ep in enumerate(eps, start=1):
                            parts = ep.split("$")
                            if len(parts) == 2:
                                pid_value = f"{vod_id_str}_{use_idx}_{idx}"
            ...
            if html:
                ...
                if data.get("code") == 200 and data.get("data"):
                    ...
                    for idx,f in enumerate(froms):
                        if f == "CO4K":
                            eps_tmp = urls[idx].split("#")
                            if eps_tmp and eps_tmp[0]:
                                target_idx = idx
                        if backup_idx == -1 and urls[idx]:
                            backup_idx = idx
                    real_src_idx = target_idx if target_idx != -1 else backup_idx
                    if real_src_idx < len(froms):
                        src_code = froms[real_src_idx]
                    if real_src_idx < len(urls):
                        eps = urls[real_src_idx].split("#")
                        if ep_idx <= len(eps):
