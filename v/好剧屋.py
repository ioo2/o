...
class Spider(BaseSpider):
    ...
    def _parse_detail(self, vid, html):
        ...
        for div in soup.select('div[id^="playlist"]'):
            ...
            if eps:
                tab_name = tab_map.get(did, did)
                all_lines.append((tab_name, eps))
        target_eps = []
        for name, eps in all_lines:
            if '腾讯' in name and len(eps) > 0:
                target_eps = eps
                break
        if not target_eps and len(all_lines) > 0:
            target_eps = all_lines[0][1]
        if not target_eps:
            target_eps = [f'正片${vid}-1-1']
        vod_play_from = '恒轩'
        vod_play_url = '#'.join(target_eps)
        return {
            'vod_id': vid,
            'vod_name': vod_name,
            'vod_pic': self._fix(pic),
