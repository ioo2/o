...
class Spider(BaseSpider):
    def __init__(self):
        ...
        self.t2s_map = {
            "鬥": "斗", "羅": "罗", "陸": "陆", "劇": "剧", "藝": "艺", "綜": "综",
            "動": "动", "畫": "画", "機": "机", "戰": "战", "鋼": "钢", "彈": "弹",
            "雲": "云", "狀": "状", "態": "态", "類": "类", "國": "国", "導": "导",
            "簡": "简", "線": "线", "賊": "贼",
        }
    ...
    def _parse_play_sources(self, html):
        ...
        target = None
        hd_list = [g for g in groups if ("HD" in g["from"] or "高清" in g["from"]) and g["urls"]]
        if hd_list:
            target = hd_list[0]
        else:
            for g in groups:
    ...
    def _parse_detail_page(self, html, vod_id):
        ...
        content = "".join(root.xpath("//div[contains(@class,'desc')]//p//text()")).strip()
        actors = []
    ...
    def _refine_search_results(self, items, keyword):
        ...
        return [i for i in items if loose_key and loose_key in self._normalize_search_keyword(i.get("vod_name", ""))]
    ...
    def _build_parse_context(self, raw_url, play_from, play_page_url):
        ...
        if play_from in ["JD4K", "JD2K", "JDHG", "JDQM"]:
            ...
            referer = f"https://play.gimyai.tw/d/?url={quote(raw_url)}&jctype={play_from}&next={quote(play_page_url)}"
        elif play_from == "NSYS":
            base = "https://play.gimyai.tw/n/"
            referer = f"https://play.gimyai.tw/n/?url={quote(raw_url)}&jctype={play_from}&next={quote(play_page_url)}"
        return {
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        if raw_url:
            ...
            try:
                ...
                parse_json = {}
            media_url = str(parse_json.get("url") or parse_json.get("video") or parse_json.get("playurl") or "").strip()
            if media_url:
                return {
                    "parse": 0,
                    "jx": 0,
