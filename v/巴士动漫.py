...
class Vod:
    ...
    def detailContent(self, did):
        ...
        try:
            ...
            vod_play_from_list = root.xpath('//ul[contains(@class, "play_from")]/li/text()')
            ...
            if len(vod_play_url) > 0:
                main_url = vod_play_url[0]
                if main_url and "$" in main_url:
                    final_from = "恒轩"
                    final_url = main_url
                else:
                    final_from = "$$$".join(["恒轩"] * len(vod_play_url))
                    final_url = "$$$".join(vod_play_url)
            video_list.append({
                'type_name': '',
                'vod_id': ids,
                'vod_name': '',
