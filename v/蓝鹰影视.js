...
async function init(cfg) {
    ...
    if (extObj.time !== undefined) {
        ...
        enableVerifyTimeSign = (timeVal === '1' || timeVal === 'true');
    }
    ...
    _.forEach(data.type_list, item => {
        ...
        _.forEach(item.filter_type_list, f => {
            ...
            if (f.name === 'class') {
                filter['name'] = '分类';
                filter['key'] = f.name;
                filter['value'] = _.filter(_.map(f.list, i => ({ v: i, n: i })), x => x.n !== '全部');
            }
            if (f.name === 'area') {
                filter['name'] = '区域';
                filter['key'] = f.name;
            ...
            if (Object.keys(filter).length > 0 && filter.value.length > 0) {
...
async function detail(id) {
    ...
    let playSources = _.map(info.vod_play_list, item => {
        ...
        const ua = playerInfo.user_agent || customPlayUa || '';
    ...
    playSources.sort((a, b) => {
        const getPriority = (show) => {
            ...
            if (show.includes('秒播')) return 5;
            if (show.includes('自建')) return 6;
            if (show.includes('蓝光')) return 7;
            if (show.includes('专线')) return 8;
            return 9;
        };
        return getPriority(a.show) - getPriority(b.show);
    });
    const sourceMap = new Map();
    for (const s of playSources) {
        const epCount = s.urls ? s.urls.split('#').length : 0;
        const rawName = s.show.replace(/\d+$/, '');
        if (!sourceMap.has(rawName)) {
            sourceMap.set(rawName, { ...s, epCount });
        } else {
            const old = sourceMap.get(rawName);
            if (epCount > old.epCount) {
                sourceMap.set(rawName, { ...s, epCount });
            }
        }
    }
    playSources = Array.from(sourceMap.values());
    let targetSource = playSources.length > 0 ? playSources[0] : { show: '恒轩', urls: '' };
    playSources = [{
        show: '恒轩',
        urls: targetSource.urls
    }];
    let showCount = {};
...
async function search(wd, quick, pg) {
    ...
    while (true) {
        ...
        if (response.code === 1001 && response.need_slider === true && !attemptedSlider) {
            ...
            if (sliderJson.code === 1 && sliderJson.data) {
                ...
                if (sliderId && targetX > 0) {
                    ...
                    let verifyResponse = await request(`${siteUrl}/${apiPrefix}/verifySlider`, verifyParams, searchHeaders, 'post');
        ...
        if (response.data && response.data.length > 0) {
            rawVideos = JSON.parse(aesDecode(response.data, key, iv)).search_list || [];
            break;
...
function jsonDanmakuToXml(json) {
    ...
    json.danmuku.forEach(danmu => {
        ...
        let [time = 0, mode = 'scroll', colorHex = '#FFFFFF', , text = '', , , fontSize = "24px"] = danmu;
...
async function proxy(params) {
    let jsonUrl = decodeURIComponent(params.url || params[0] || params.danmaku || '');
