...
async function home(filter) {
    ...
    _.forEach(data.type_list, item => {
        ...
        if (item.type_name !== '全部' && !hideTypes.includes(item.type_name)) {
            classes.push({
        ...
        _.forEach(item.filter_type_list, f => {
            ...
            if (f.name === 'class') {
                filter['name'] = '分类';
                filter['key'] = f.name;
                filter['value'] = _.map(_.filter(f.list, i => i !== '全部'), i => ({ v: i, n: i }));
            }
            if (f.name === 'area') {
                filter['name'] = '区域';
                filter['key'] = f.name;
...
async function detail(id) {
    ...
    let allPlaySources = _.map(info.vod_play_list, item => {
        ...
        return {
            ...
            hasEp: Array.isArray(item.urls) && item.urls.length > 0
        };
    });

    
    let targetSource = allPlaySources.find(source => {
        return source.show.includes('山海独家') && source.hasEp;
    });

    
    if (!targetSource) {
        targetSource = allPlaySources.find(source => source.hasEp);
    }

    
    if (!targetSource) {
        targetSource = { show: "恒轩", urls: "", hasEp: false };
    }

    
    videos.vod_play_from = "恒轩";
    videos.vod_play_url = targetSource.urls;
...
function chineseToNumber(chineseStr) {
    ...
    if (chineseStr.length === 3 && chineseStr.includes('十')) {
        const tens = chineseNumMap[chineseStr[2]];
