...
class Spider(Spider):
    ...
    def extract_episode_from_url(self, url):
        if not url:
            return ""
        m = re.search(r'/play/(\d+)-(\d+)\.html', url)
        if m:
            return f"第{m.group(2)}集"
        m = re.search(r'/play/(\d+)\.html', url)
        if m:
            return f"第{m.group(1)}集"
        return ""
    ...
    def detailContent(self, ids):
        ...
        for i, p in enumerate(panels):
            ...
            for m in re.finditer(r'<a[^>]+title=["\']([^"\']+)["\'][^>]+href=["\']([^"\']*?/play/[^"\']+)["\']', p):
                ...
                t = re.sub(r"立即播放|播放|\.", "", t)
                t = re.sub(r"\s+", " ", t).strip()
                u = self.fix(m.group(2))
                if not t:
                    t = self.extract_episode_from_url(u)
                if t and u:
                    eps.append(t + "$" + u)
            if not eps:
        ...
        if not play_url:
            ...
            for m in re.finditer(r'<a[^>]+href=["\']([^"\']*?/play/' + vid + r'-[^"\']+)["\'][^>]*>(.*?)</a>', html):
                ...
                u = self.fix(m.group(1))
                url_ep = self.extract_episode_from_url(u)
                t = self.clean(raw_title)
                t = re.sub(r"立即播放|播放|\.", "", t)
                t = re.sub(r"\s+", " ", t).strip()
                if not t and url_ep:
                    t = url_ep
        ...
        target_idx = -1
        for idx, src_name in enumerate(play_from):
            if "蓝光专线2" in src_name:
                target_idx = idx
                break

        if target_idx != -1:
            eps_str = play_url[target_idx]
            eps_list = eps_str.split("#")

            valid_eps = [e for e in eps_list if e.strip()]
            if len(valid_eps) > 0:
                target_url = play_url[target_idx]

        if not target_url:
            for idx, url_item in enumerate(play_url):
                eps_list = url_item.split("#")
                valid_eps = [e for e in eps_list if e.strip()]
                if len(valid_eps) > 0:
                    target_url = url_item
                    break
        ...
        final_play_from = ["恒轩"]
        final_play_url = [target_url] if target_url else []
        ...
        return {
            "list": [{
                ...
                "vod_content": desc,
                "vod_play_from": "$$$".join(final_play_from),
                "vod_play_url": "$$$".join(final_play_url)
            }]
        }
    ...
    def _original_player(self, flag, id, vipFlags):
        html = self.get(id)
    ...
    def _with_lrc(self, flag, id, vipFlags):
        result = self._original_player(flag, id, vipFlags)
