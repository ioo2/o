        ...
        try:
            ...
            for tab, box in zip(tabs, boxes):
                ...
                if links:
                    ...
                    play_str = "#".join([f"{ep}${urljoin(HOST, href)}" for ep, href in zip(eps, hrefs)])
                    source_list.append({"name":tab, "play": play_str})
            if not source_list:
                ...
                for route in sorted(routes.keys(), key=lambda x: int(x) if x.isdigit() else 999):
                    name = tabs[route_idx] if route_idx < len(tabs) else f"{route}线"
                    play_str = "#".join(routes[route])
                    source_list.append({"name":name, "play": play_str})
                    route_idx += 1
            selected_play = ""
            aa_item = next((s for s in source_list if s["name"] == "AA线"), None)
            if aa_item and aa_item["play"]:
                selected_play = aa_item["play"]
            else:
                valid = [s for s in source_list if s["play"]]
                if valid:
                    selected_play = valid[0]["play"]
            if selected_play:
                d["vod_play_from"] = "恒轩"
                d["vod_play_url"] = selected_play
        except:
            pass
        return {"list": [d]}
