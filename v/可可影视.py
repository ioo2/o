    ...
    def detailContent(self, ids):
        ...
        try:
            ...
            for i, sid in enumerate(sids_in_order):
                if sid not in episodes_by_sid or not episodes_by_sid[sid]:
                    continue
            ...
            if not target_line and len(all_source) > 0:
                target_line = all_source[0]
        
            play_from = []
            play_url = []
            if target_line:
                play_from.append("恒轩")
                play_url.append(target_line['data'])
