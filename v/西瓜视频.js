...
function _selectPlaySource(playFromStr, playUrlStr) {
    const fromArr = playFromStr ? playFromStr.split("$$$") : [];
    const urlArr = playUrlStr ? playUrlStr.split("$$$") : [];
    let txGroup = [];
    let otherGroup = [];

    for (let i = 0; i < fromArr.length; i++) {
        const name = fromArr[i] || "";
        const urlItem = urlArr[i] || "";
        if (urlItem && urlItem.length > 0) {
            if(name.toUpperCase().includes("TX")){
                txGroup.push({ name, url: urlItem });
            }else{
                otherGroup.push({ name, url: urlItem });
            }
        }
    }
   
    const backupSources = [...txGroup, ...otherGroup];
    const selectSource = backupSources[0] || { name: "", url: "" };

    return {
        playFrom: "恒轩",
        playUrl: selectSource.url || "",
        backupSources: backupSources
    }
}

async function init(cfg) { }
const orderFilter = { "key": "orderby", "name": "排序", "value": [{ "n": "默认排序", "v": "time" }, { "n": "人气", "v": "hits" }, { "n": "时间", "v": "time" }, { "n": "评分", "v": "score" }] };
const myFilters = {
...
async function category(tid, pg, filter, extend) {
    const page = pg || "1";
    const sortVal = extend.orderby || "time";
    const url = `${HOST}/index.php/vod/show/by/${sortVal}/id/${extend.tid || tid}/page/${page}.html`;
    try {
...
async function detail(id) {
    try {
        ...
        const sourceResult = _selectPlaySource(playFromList.join("$$$"), playUrlList.map(e => e.join("#")).join("$$$"));
        return JSON.stringify({
            list: [{
                vod_id: id,
                ..._parseVodInfo(html),
                vod_play_from: sourceResult.playFrom,
                vod_play_url: sourceResult.playUrl,
                _backup_sources: sourceResult.backupSources
            }]
        });
    } catch (e) { return errRes(e.message); }
}
    ...
    try {
        ...
        if (match) {
            ...
            if (token) {
                ...
                mylog("解析结果:", res2);
                if (res2.code ==404 ) {
                    return JSON.stringify({msg:'播放失败'});
                }
                const finalUrl = formatUrl(res2.url);
                return JSON.stringify({ parse: 0, url: finalUrl, header: HEADERS });
            }
        }
