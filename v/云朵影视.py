...
版本: 1.0.1 【修改：仅单线路，优先站外-如意，线路名恒轩】
"""
import sys
import json
import re
...
class Spider(Spider):
    ...
    def detailContent(self, ids):
        ...
        ruyi_idx = -1
        for idx, name in enumerate(play_from_list):
            if "站外 - 如意" in name:
                ruyi_idx = idx
                break
        if ruyi_idx != -1:
            final_url = play_url_list[ruyi_idx]
        else:

            if len(play_url_list) > 0:
                final_url = play_url_list[0]
            else:
                # 兜底线路
                final_url = "正片$" + vod_id

        vod['vod_play_from'] = "恒轩"
        vod['vod_play_url'] = final_url
        return {'list': [vod]}
    # ========== 搜索 ==========
